...
{
    public class EasyCachingExpressionInterceptor : AbstractInterceptor
    {/// <summary>
     ...
     /// <value>The cache provider.</value>
        ...
        [FromServiceContext]
        public IEasyCachingExpressionKeyGenerator KeyGenerator { get; set; }
        ...
        /// </summary>
        [FromServiceContext]
        public ILogger<EasyCachingExpressionInterceptor> Logger { get; set; }
            ...
            if (GetMethodAttributes(context.ServiceMethod).FirstOrDefault(x => typeof(EasyCachingEvictAttribute).IsAssignableFrom(x.GetType())) is EasyCachingEvictAttribute attribute && attribute.IsBefore == isBefore)
            {
                try
                {
                    if (attribute.IsAll)
                    {
                        ...
                        var cachePrefix = KeyGenerator.GetCacheKeyPrefix(context.ServiceMethod, context.Parameters, attribute.CacheKeyPrefix);
